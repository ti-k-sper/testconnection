			...
			if (!$user){//si l'utilisateur n'existe pas
				...
				if (strlen($password) <= 10 && strlen($password) >= 5) {//vérif longueur mdp
					...
					if ($password === $passwordVerif) {//vérif mdp
						...
						require_once 'db.php';
						$sql = 'INSERT INTO `user`(`name`, `password`) VALUES (:name, :password)';// /!\ ne pas confondre value de input
						$statement = $pdo->prepare($sql);
		...
		<section class="login-container">
			<div>
				...
				<form action="" method="POST">
					...
					<input type="password" name="password_verif" placeholder="Confirmez le mot de passe" required="required">
